...
const brochuresData = [
  { "school": "國立臺灣大學", "url": "https://exam.aca.ntu.edu.tw/graf/brochure/detail.asp" },
...
async function updateBrochures() {
  console.log('🚀 Starting Brochure URL Updates...');

  let successCount = 0;
  let missingCount = 0;

  for (const item of brochuresData) {
    let searchName = item.school
      .replace(/國立/g, '')
      .replace(/私立/g, '')
      .replace(/台北/g, '臺北')
      .replace(/台中/g, '臺中')
    ...
    if (!schools || schools.length === 0) {
      console.warn(`⚠️ School not found in DB: ${item.school} (searched as: ${searchName})`);
      missingCount++;
      continue;
    }
    ...
    if (!programs || programs.length === 0) {
      console.warn(`⚠️ No programs found for ${item.school}`);
      continue;
    }

    console.log(`📝 Updating ${programs.length} programs for ${item.school}...`);

    for (const program of programs) {
      // Upsert standard 115th year application row with just the source URL
  ...
  console.log(`✅ Processed ${successCount} schools. ${missingCount} missing.`);
}
